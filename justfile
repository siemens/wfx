...
build:
    goreleaser build --id wfx --id wfxctl --clean --single-target --snapshot

build-wfx-with-ui:
    goreleaser build --id wfx-with-ui --snapshot --clean --single-target
...
_generate-openapi:
    ...
    set -eu
    cd "{{ THISDIR }}/spec"
    go tool -modfile="{{ THISDIR }}/tools/go.mod" github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen -config cfg.yaml wfx.openapi.yml
...
_generate-ent:
    ...
    go tool -modfile="{{ THISDIR }}/tools/go.mod" entgo.io/ent/cmd/ent generate --header \
...
_generate-mockery:
    go tool -modfile="{{ THISDIR }}/tools/go.mod" github.com/vektra/mockery/v3
...
_generate-flatbuffers:
    ...
    go tool -modfile="{{ THISDIR }}/tools/go.mod" mvdan.cc/gofumpt -l -w generated/plugin
