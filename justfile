...
pages:
    ...
    popd
    mv hugo/public .
    go tool -modfile="{{ THISDIR }}/go.tool.mod" github.com/wjdp/htmltest
    lychee public
...
format:
    #!/usr/bin/env bash
    set -eux
    go tool -modfile="{{ THISDIR }}/go.tool.mod" mvdan.cc/gofumpt -l -w .
    prettier -l -w .
    just --fmt --unstable
...
_generate-openapi:
    ...
    set -eu
    cd "{{ THISDIR }}/spec"
    go tool -modfile="{{ THISDIR }}/go.tool.mod" github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen -config cfg.yaml wfx.openapi.yml
...
_generate-ent:
    ...
    go tool -modfile="{{ THISDIR }}/go.tool.mod" entgo.io/ent/cmd/ent generate --header \
...
_generate-mockery:
    go tool -modfile="{{ THISDIR }}/go.tool.mod" github.com/vektra/mockery/v3
...
_generate-flatbuffers:
    ...
    go tool -modfile="{{ THISDIR }}/go.tool.mod" mvdan.cc/gofumpt -l -w generated/plugin
