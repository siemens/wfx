...
@external(javascript, "./auth.ffi.mjs", "currentUser")
fn current_user_tuple() -> #(String, String, String) {
  #("", "", "")
}

pub fn current_user() -> Option(#(String, String, String)) {
  case current_user_tuple() {
    #("", "", "") -> None
    user -> Some(user)
  }
}

@external(javascript, "./auth.ffi.mjs", "logout")
fn do_logout() -> Nil {
  Nil
}

pub fn logout() -> Effect(message) {
  effect.from(fn(_) { do_logout() })
}
