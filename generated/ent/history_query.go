...
func (hq *HistoryQuery) QueryJob() *JobQuery {
	query := (&JobClient{config: hq.config}).Query()
...
func (hq *HistoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*History, error) {
	...
	if query := hq.withJob; query != nil {
		if err := hq.loadJob(ctx, query, nodes, nil,
			func(n *History, e *Job) { n.Edges.Job = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (hq *HistoryQuery) loadJob(ctx context.Context, query *JobQuery, nodes []*History, init func(*History), assign func(*History, *Job)) error {
