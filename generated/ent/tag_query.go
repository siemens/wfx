...
func (_q *TagQuery) QueryJobs() *JobQuery {
...
func (_q *TagQuery) WithJobs(opts ...func(*JobQuery)) *TagQuery {
	query := (&JobClient{config: _q.config}).Query()
...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := _q.withJobs; query != nil {
		if err := _q.loadJobs(ctx, query, nodes,
			func(n *Tag) { n.Edges.Jobs = []*Job{} },
			func(n *Tag, e *Job) { n.Edges.Jobs = append(n.Edges.Jobs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TagQuery) loadJobs(ctx context.Context, query *JobQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Job)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.JobsTable)
		s.Join(joinT).On(s.C(job.FieldID), joinT.C(tag.JobsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.JobsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.JobsPrimaryKey[0]))
