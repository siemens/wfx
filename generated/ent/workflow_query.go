...
func (wq *WorkflowQuery) loadJobs(ctx context.Context, query *JobQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *Job)) error {
	...
	query.Where(predicate.Job(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.JobsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.workflow_jobs
		if fk == nil {
			return fmt.Errorf(`foreign-key "workflow_jobs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workflow_jobs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
