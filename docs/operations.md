...
This is a deliberate design decision to keep things simple and to not artificially restrict possible use cases.
Nonetheless, e.g., with header authentication and the X-Client-Id header, this feature can be restricted as follows.

A trusted API gateway can authenticate each southbound client and ensure that the `X-Client-Id` header contains that client's authenticated ID before forwarding the request to wfx.
wfx uses this optional header for these southbound operations:
