...
func TestUDS(t *testing.T) {
	...
	cmd.SetArgs([]string{
		...
		"--scheme=unix",
		fmt.Sprintf("--client-unix-socket=%s", clientSocket),
		fmt.Sprintf("--mgmt-unix-socket=%s", mgmtSocket),
	})
	ctx, cancel := context.WithCancel(context.Background())
	cmd.SetContext(ctx)

	cmdErr := errors.New("Should be set to nil by the goroutine")
...
func TestAPI(t *testing.T) {
	...
	t.Run("/openapi.json", func(t *testing.T) {
		...
		var spec openapi3.T
		for i := 0; i < 100; i++ {
			resp, err := httpClient.Get("http://localhost:8080/openapi.json")
			if err != nil {
				time.Sleep(time.Millisecond * 10)
				continue
			}
	...
	t.Run("Fileserver Disabled", func(t *testing.T) {
		...
		assert.Equal(t, `{"code":404,"message":"path /download was not found"}`, string(body))
	})
